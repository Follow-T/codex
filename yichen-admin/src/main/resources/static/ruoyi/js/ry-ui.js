	...
	$
			.extend({
				...
				table : {
					...
					init : function(options) {
						...
						$('#' + options.id).bootstrapTable({
							...
							sortOrder : options.sortOrder, // 排序方式 asc 或者
							// desc
							...
							iconSize : 'outline', // 图标大小：undefined默认的按钮尺寸
							// xs超小按钮sm小按钮lg大按钮
					...
					responseHandler : function(res) {
						if (res.code == 0) {
							if ($.common.isNotEmpty($.table._option.sidePagination)
									&& $.table._option.sidePagination == 'client') {
								return res.rows;
							} else {
								return {
									rows : res.rows,
					...
					onLoadSuccess : function(data) {
						...
						$("#" + $.table._option.id).on("check.bs.table uncheck.bs.table check-all.bs.table uncheck-all.bs.table load-success.bs.table", function() {
							var ids = $("#" + $.table._option.id).bootstrapTable(
									"getSelections");
							$('#' + $.table._option.toolbar + ' .btn-del')
									.toggleClass('disabled', !ids.length);
							$('#' + $.table._option.toolbar + ' .btn-edit')
									.toggleClass('disabled', ids.length != 1);
						});
					},
					// 表格销毁
					...
					exportExcel : function(formId) {
						...
						$.post($.table._option.exportUrl, $("#" + currentId).serializeArray(), function(result) {
							if (result.code == web_status.SUCCESS) {
								window.location.href = ctx + "common/download?fileName="
										+ encodeURI(result.msg) + "&delete=" + true;
							} else {
				...
				modal : {
					...
					editOpen : function() {
						...
						parent.layer.open({
							...
							title : title,
							content : url,
							btn : btnArray, // 例如['取消', '通过一级稽核','确定']
							/*
							 * yes: function(index, layero) { // 按钮【取消】的回调 layer.close(index); },
							 */
							btn2 : callback1, // 按钮【确定】的回调
							btn3 : callback2,// 按钮【通过一级稽核 / 通过二级稽核 / 中间的按钮】的回调
							success : successCallBack, // 弹出后的成功回调方法
				...
				operate : {
					...
					// 清空信息
					clean : function() {
						$.modal.confirm("确定清空所有" + $.table._option.modalName + "吗？",
								function() {
